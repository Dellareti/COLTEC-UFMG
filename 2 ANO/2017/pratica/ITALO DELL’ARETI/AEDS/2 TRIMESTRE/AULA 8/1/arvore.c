...
void busca(Arv* a,char nome[]){
	if(!arv_vazia(a)){
		if(strcmp(a->contato.nome,nome)==0){
			printf("Informações do contato : %s\n",nome);
			printf("----\nNome: %s\nTel: %s\nEnd: %s\n",a->contato.nome,a->contato.telefone,a->contato.endereco);
		}
		busca(a->esq,nome);
		busca(a->dir,nome);
	}
...
Arv *abb_retira(Arv *r,int v){
	if(r==NULL)
		...
		//elemento tem filho a direita
		else if(r->esq==NULL){
			Arv *t = r;
			r = r->dir;
			free(t);
		}
		//elemento tem filho a esquerda
		else if(r->dir==NULL){
			Arv *t =r;
			r = r->esq;
			free(t);
