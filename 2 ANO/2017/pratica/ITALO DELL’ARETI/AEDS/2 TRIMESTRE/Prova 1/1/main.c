#include <stdio.h>
#include "grafo.h"


int main(){
	Grafo novo_grafo = cria_grafo(7,0,1);
	
	novo_grafo = faz_ligacao(novo_grafo,1,3,1);
	novo_grafo = faz_ligacao(novo_grafo,1,4,1);
	novo_grafo = faz_ligacao(novo_grafo,1,5,1);
	novo_grafo = faz_ligacao(novo_grafo,2,3,1);
	novo_grafo = faz_ligacao(novo_grafo,2,4,1);
	novo_grafo = faz_ligacao(novo_grafo,2,5,1);
	novo_grafo = faz_ligacao(novo_grafo,3,5,1);
	novo_grafo = faz_ligacao(novo_grafo,4,5,1);
	

	imprime_informacoes_grafo(novo_grafo);
	libera_grafo(novo_grafo);
}
