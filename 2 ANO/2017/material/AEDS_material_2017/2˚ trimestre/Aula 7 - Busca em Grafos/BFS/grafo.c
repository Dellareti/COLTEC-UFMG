...
Grafo *busca_grafo(Grafo *g,int valor){
	...
	for(i=0;i<g->n_adjacentes;i++){
		fila_insere_l(fila,g->adjacentes[i]);
		g->adjacentes[i]->estado=2;
	}

	Grafo *auxiliar;
	while(!fila_vazia_l(fila)){
		auxiliar = fila_retira_l(fila);
		if(auxiliar->v==valor){
			printf("encontrei!\n");
			return g;
		}

		for(i=0;i<auxiliar->n_adjacentes;i++){
			if(auxiliar->adjacentes[i]->estado==0){
				auxiliar->adjacentes[i]->estado=2;
				fila_insere_l(fila,auxiliar->adjacentes[i]);
			}
		}
	}
	printf("NOT FOUND\n");
