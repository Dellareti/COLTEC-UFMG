...
Lista *lst_retira(Lista *l, int v){
	...
	if (p == NULL)
		return l; // elemento n˜ao encontrado, retorna lista original
...
int lst_igual(Lista *l1, Lista *l2){
	...
	for(N = l1; N != NULL ; N = N->prox, N2 = N2->prox) { 
		if(N->info != N2->info) 
			return 0 ; 
	} 
...
Lista2 *lst2_insere(Lista2 *l, int v){
	...
	novo->info = v;
	novo->prox = l;
	novo->ant = NULL; //inserindo no in´ıcio da lista
...
Lista2 *lst2_retira(Lista2 *l, int v){
	...
	Lista2 *p = lst2_busca(l, v);
		...
		if (p == NULL)
			return l; //elemento n˜ao encontrado
		...
		if (p->prox != NULL) //testa se ´e o ´ultimo elemento da lista
			p->prox->ant = p->ant;
